...
const CustomLink = ({ href, title, className = "" }) => {
  ...
  // console.log(router) // it provide different objects
  return (
    ...
    <header className='w-full px-32 py-8 font-medium flex items-center justify-between
     dark:text-light relative z-10 lg:px-16 sm:px-8'>
      ...
      <button className='flex-col justify-center items-center hidden lg:flex' onClick={handleClick}>
        ...
        <span className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm  ${isOpen ? '-rotate-45 -translate-y-1' : 'transition-all duration-300 ease-out translate-y-0.5'}`}></span>
      </button>
      ...
      <div className='w-full flex justify-between items-center lg:hidden'>
        <nav>
          ...
          <CustomLink href='/articles' title="Key Highlights" className='ml-4' />
          <CustomLink href='/books' title="Blogs" className='ml-4' />
        </nav>
